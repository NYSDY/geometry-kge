...
class TestNormal:
    ...
    def prepare(self, final_test=False):
        ...
        f1_df = pd.read_csv("../vector/" + self.data_set + "/entity2vec.vec", header=None, sep='\t')
        f2_df = pd.read_csv("../vector/" + self.data_set + "/relation2vec.vec", header=None, sep='\t')
        self.entity_vec = []
...
def test_triple(data_set):
    test_normal = TestNormal(data_set)
