...
class LinkPrediction:
    def __init__(self, kg: KnowledgeGraph, score_func,
        ...
        '''读入数据'''
        f1_df = pd.read_csv("../vector/"+ kg.data_dir +"/entity2vec.vec", header=None, sep='\t')
        f2_df = pd.read_csv("../vector/"+ kg.data_dir +"/relation2vec.vec", header=None, sep='\t')
        entity_vec = []
...
def test_begain():
    kg=KnowledgeGraph('YAGO39K')
    lk = LinkPrediction(kg, "L2", 100, 32)
    gpu_config = tf.GPUOptions(allow_growth=True)
    sess_config = tf.ConfigProto(gpu_options=gpu_config)
    with tf.Session(config=sess_config) as sess:
        tf.global_variables_initializer().run()
        lk.launch_evaluation(sess)
