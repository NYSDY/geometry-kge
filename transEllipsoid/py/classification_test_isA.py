...
class TestIsA:
    def __init__(self, data_set, epoch):
    ...
    def load_vector(self):
        ...
        f2 = open("../vector/" + self.data_set + "/concept2vec" + str(self.epoch) + ".vec", 'r', encoding='utf-8')
        ...
        self.concept_vec = np.array(self.concept_vec)
    ...
    def test(self):
        ...
        def check_instance(instance, concept):
            dis = np.sum(np.square(np.true_divide(self.instance_vec[instance] - self.concept_vec[concept],
                                           self.concept_r[concept][:-1])))
            score = dis - np.square(self.concept_r[concept][-1])
        ...
        def check_sub_class(concept1, concept2):
            dis = np.sum(np.square(np.true_divide(self.concept_vec[concept1], self.concept_r[concept1][:-1]) -
                            np.true_divide(self.concept_vec[concept2], self.concept_r[concept2][:-1])))
            if np.sqrt(dis) < np.fabs(self.concept_r[concept1][-1] - self.concept_r[concept2][-1]) and \
                    self.concept_r[concept1][-1] < self.concept_r[concept2][-1]:
                return True
            if np.sqrt(dis) < self.concept_r[concept1][-1] + self.concept_r[concept2][-1]:
                tmp = (self.concept_r[concept1][-1] + self.concept_r[concept2][-1] - np.sqrt(dis)) / self.concept_r[concept1][-1]
...
def test_isA(data_set, epoch):
    test_isa_example = TestIsA(data_set, epoch)
    result_tuple = test_isa_example.run()
    return result_tuple


if __name__ == '__main__':
    test_isA('YAGO39K', 1500)